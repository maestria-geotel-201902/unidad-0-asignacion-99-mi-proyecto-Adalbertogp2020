...
proven17 %>%
  dplyr::select(contains('A qué cree que se debe la delincuencia en el país: ¿A la falta de lugares para practicar deportes?: Si')) %>%
  plot(breaks = 'jenks')
```
...
```{r}
mivariable <- 'A qué cree que se debe la delincuencia en el país: ¿A la falta de lugares para practicar deportes?: Si'
proven17_mivar <- proven17 %>%
  st_centroid() %>% 
  select(ENLACE, mivariable=contains(mivariable), muestra) %>% 
  mutate('mivariable_pct' = mivariable/muestra*100,
...
#1-para la variable original=prueba de Shapiro-Wilk resulta no significativa (es decir, el valor de *p* mayor que 0.05), entonces se asume como válido el supuesto de normalidad.
#2-para la variable modificada=prueba de Shaprio-Wilk resulta significativa (es decir, el valor de *p*menor que 0.05), entonces se asume como no válido el supuesto de normalidad.
...
```{r}
(gmoranw <- moran.test(x = proven17_mivar_sf$mivariable_pct, listw = proven17.w.W))
(gmoranb <- moran.test(x = proven17_mivar_sf$mivariable_pct, listw = proven17.w.B))
```

* Interpreta el resultado de la comprobación anterior aquí:
#1-para los pesos estandarizados=el valor de *p* obtenido fue mayor de 0.05,no hay evidencia preliminar para rechazar la hipótesis nula de "no hay autocorrelación espacial", y por lo tanto concluir que "no hay autocorrelación espacial"
#2-para los pesos binarios=el valor de *p* obtenido fue mayor de 0.05,no hay evidencia preliminar para rechazar la hipótesis nula de "no hay autocorrelación espacial", y por lo tanto concluir que "no hay autocorrelación espacial"
...
```{r}
moran.plot(x = proven17_mivar_sf$mivariable_pct, listw = proven17.w.W)
source('lisaclusters.R')
lisamap(objesp = proven17_mivar_sf,
        var = 'mivariable_pct',
        pesos = proven17.w.W,
